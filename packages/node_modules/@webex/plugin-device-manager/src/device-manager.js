...
const DeviceManager = SparkPlugin.extend({
        ...
        return this._updateDeviceMetadata(res.body.items).then((devices) => {
          ...
          devices.forEach((device) => {
            if (device.deviceInfo && device.deviceInfo.machineType === LYRA_SPACE) {
              return this.spark.internal.lyra.space.get(device.deviceInfo).catch((err) => {
  ...
  search(options) {
    ...
    return this.spark.internal.search
      .people({
        searchId: uuid.v4(),
  ...
  get(token) {
    ...
    return this.spark.internal.lyra
  ...
  requestPin(device, options = {}) {
    ...
    return this.spark.internal.lyra.space
      ...
      .then((dev) => {
        ...
        return this.spark.internal.lyra.space.get(space).then(() => Promise.resolve(dev));
      })
      .catch((err) => {
  ...
  unpair(options = {}) {
    ...
    return this.spark.internal.lyra.space.leave(space, options).catch((err) => {
  ...
  bindSpace(options = {}) {
    ...
    return this.spark.internal.lyra.space.bindConversation(space, this._boundSpace).catch((err) => {
  ...
  _updateDeviceMetadata(deviceArray = []) {
    ...
    return Promise.all(
      devices.map((device, index) => {
        ...
        return this._decryptDeviceName(device).then((updatedDevice) => {
  ...
  _decryptDeviceName(inDevice = {}) {
    ...
    if (device.metadata && device.metadata.encryptedUserAssignedName && device.metadata.encryptionKeyUrl) {
      return this.spark.internal.encryption
        ...
        return this.spark.internal.lyra.space
          ...
          .then((space) => {
            ...
            if (device && space.occupants && (!space.occupants.self || !space.occupants.self.verified)) {
              device.productName =
                (space.devices && space.devices[0] && space.devices[0].productName) || DEFAULT_PRODUCT_NAME;
