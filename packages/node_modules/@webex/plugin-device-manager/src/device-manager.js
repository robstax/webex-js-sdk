...
const DeviceManager = SparkPlugin.extend({
  ...
  initialize() {
    ...
    this.spark.internal.mercury.on('event:lyra.space_updated', ({data}) => {
  ...
  requestPin(code) {
    this.spark
      .request({
        method: 'GET',
        api: 'proximity',
        resource: `/discoverycode/advertisements/${code}`
      })
      .then((res) => res.body)
      .then((res) =>
        this.spark.internal.lyra.space
          .join(
            {},
            {
              passType: 'PROOF',
              ...
              data: res.proof,
              uri: res.links.addMeToSpace.href
            }
          )
          .then(() => this.spark.internal.lyra.space.get({id: res.advertiser}).then((newDevice) => {
            this._devicePendingPinChallenge = newDevice;
  ...
  */
  pair(pin) {
    if (!pin) {
      console.error('must provide pin');

      return Promise.reject(new Error('must provide pin'));
    }
    if (this._devicePendingPinChallenge) {
      return this.spark.internal.lyra.space
        .join(
          {},
          {
            passType: 'PIN_ANSWER',
            data: pin,
            uri: this._devicePendingPinChallenge.occupants.links.addMeToSpace.href
          }
        )
        .then(() => this.spark.internal.lyra.space.get({id: this._devicePendingPinChallenge.identity.id}).then((newDevice) => {
          this._pairedDevice = newDevice;

          return newDevice;
        }))
        .catch((err) => {
          console.error('pair: incorrect PIN, unable to pair ', err);

          return Promise.reject(err);
        });
    }
    console.error('pair: no device to pair');

    return Promise.reject(new Error('pair: no device to pair'));
  },
  ...
  */
  bindSpace(options = {}) {
    ...
    return this.spark.internal.lyra.space.bindConversation(space, this._boundSpace)
      .catch((err) => {
  ...
  */
  _updateDeviceMetadata(deviceArray = []) {
    ...
    return Promise.all(devices.map((device, index) => {
  ...
  */
  _receiveDeviceUpdates(device) {
    ...
    if (device && device.spaceUrl) {
      ...
      if (existingDevice) {
        return this.spark.internal.lyra.space.get({id: deviceId})
          .then((space) => {
