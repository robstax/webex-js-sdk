...
const DeviceManager = SparkPlugin.extend({
  ...
  */
  anonRequestPin(code) {
    if (!code) {
      this.logger.error('DeviceManager#anonRequestPin: device code is required');

      return Promise.reject(new Error('DeviceManager#requestPin: device.id is required'));
    }

    return this.spark.internal.lyra.getDiscoveryCodeEndpoint(code)
      .then((res) => this.spark.internal.lyra.space.join(
        {},
        {
          passType: 'PROOF',
        ...
        this._devicePendingPinChallenge = device;

        return device;
      }))
      .catch((err) => {
        this.logger.error('DeviceManager#requestPin: device failed anon PIN challenge request', err);
