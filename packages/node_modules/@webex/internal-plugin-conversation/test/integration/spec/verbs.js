...
describe('plugin-conversation', function () {
  ...
  describe('verbs', () => {
    ...
    describe('verbs that update objects', () => {
      ...
      describe('#react()', () => {
        let addReactionPayload, activity, deleteReactionPayload, reactionId;

        before(() =>
          webex.internal.conversation.post(conversation, {displayName: 'React Me 1'}).then((a) => {
            activity = a;
            addReactionPayload = {
              actor: {objectType: 'person', id: spock.id},
              target: {
                id: conversation.id,
                objectType: 'conversation'
              },
        ...
        it('reacts and deletes a post', () => webex.internal.conversation.react(conversation, addReactionPayload)
          .then((r) => {
            reactionId = r.id;
          ...
          .then((c) => {
            const selfSummary = c.activities.items[1].children.find((child) => child.type === 'reactionSelfSummary');

            assert.equal(selfSummary.activity.object.objectType, 'reaction2SelfSummary');
            assert.equal(selfSummary.activity.object.reactions.length, 1);
            ...
            return webex.internal.conversation.react(conversation, deleteReactionPayload);
          }).then(() => webex.internal.conversation.get(conversation, {
            activitiesLimit: 5,
            includeParticipants: true,
            includeChildren: true
          ...
          .then((c) => {
            // this depends on timing of convo service
            // if convo service was quick and aggregated a reactionSummary, there will be children (a reactionSummary with no reactions)
            // if convo service was slow to aggregate, there will be no children, since the selfSummary has been deleted
            if (c.activities.items[1].children) {
              assert.isNull(c.activities.items[1].children.find((child) => child.type === 'reactionSelfSummary'), null);
            }
            else {
              assert.isUndefined(c.activities.items[1].children);
            }
          }));
      });
