  ...
  describe('when interacting with a non-encrypted conversation', () => {
    ...
    describe('when the conversation is a grouped conversation', () => {
      ...
      describe('#addReaction()', () => {
        ...
        beforeEach(() =>
          webex.internal.conversation.post(conversation, {displayName: 'Ahoy'}).then((a) => {
            activity = a;

            // consume the mercury event for posting the comment
            return Promise.resolve(expectEvent(10000, 'event:conversation.activity', checkov.webex.internal.mercury, 'spock should get the presence active event'));
            // return Promise.all([
            //   expectEvent(10000, 'event:conversation.activity', checkov.webex.internal.mercury, 'spock should get the presence active event')
            // ]);
          }));
        it('reacts to message', () => webex.internal.conversation.addReaction(conversation, 'smiley', {
          type: 'reaction',
          id: activity.id,
          ...
          .then((event) => assert.equal(event.data.activity.object.reactions[0].displayName, 'smiley'))
          // we know it'safe to get convo now, since we got the event
          .then(() => checkov.webex.internal.conversation.get(conversation, {activitiesLimit: 10, includeChildren: true}))
          .then((c) => {
            assert.property(c, 'defaultActivityEncryptionKeyUrl');
            assert.equal(c.activities.items[3].object.reactions[0].displayName, 'smiley');
          }));
      });
