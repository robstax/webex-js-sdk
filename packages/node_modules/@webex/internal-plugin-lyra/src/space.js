 ...
 * @memberof Lyra
 */
const Space = SparkPlugin.extend({
  namespace: 'Lyra',
  ...
  join(space, options) {
    ...
    const body = {
      ...
      deviceUrl: process.env.CEC_DEVICE_URL
    };
  ...
  leave(space, options = {}) {
    ...
    if (!options.removeAllDevices) {
      const params = {
        deviceUrl: base64.toBase64Url(this.spark.internal.device.url)
      };
  ...
  unbindConversation(space = {}, conversation = {}, options = {}) {
    ...
    return this.spark.internal.encryption.kms.prepareRequest(parameters.kmsMessage).then((req) => {
