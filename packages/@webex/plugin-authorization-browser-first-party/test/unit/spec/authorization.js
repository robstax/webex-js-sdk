...
describe('plugin-authorization-browser-first-party', () => {
  describe('Authorization', () => {
    ...
    describe('#handleThirdPartyCallback()', () => {
      ...
      it('returns IdBroker SocialLoginError value in error field, leaves error in URL, with empty state', () => {
        const storedToken = 'csrf-abc';
        const encodedState = buildState({csrf_token: storedToken});
        // IdBroker emits `error` (SocialLoginError enum string) on failure;
        // there is no `errorCode` param. See ThirdPartyLoginServlet.java
        // in CommonIdentity/identity-broker.
        const search = `?error=FailedToCallOAuthProvider&state=${encodedState}`;
        // Construct webex with a benign href to avoid initialize()'s
        // _checkForErrors throwing during MockWebex setup, then swap the
        // href to the callback URL before invoking handleThirdPartyCallback.
        ...
        return webex.authorization.handleThirdPartyCallback(search).then((result) => {
          ...
          // `error` is non-sensitive (matches /authorize's behaviour of
          // leaving OAuth-spec error strings in the URL); not scrubbed.
