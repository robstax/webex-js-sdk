    ...
    describe('#initialize', () => {
      describe('when requireServiceCatalogOnReady is disabled (default)', () => {
        it('initFailed is false when initialization succeeds and credentials are available', async () => {
          // Store original listenToOnce to track nested calls
          const originalListenToOnce = services.listenToOnce.bind(services);
          let readyCallback;

          services.listenToOnce = sinon.stub().callsFake((target, event, callback) => {
            if (event === 'change:config') {
              // Immediately invoke change:config callback to set up ready listener
              // Set up config first (requireServiceCatalogOnReady is false/undefined by default)
